...
@section('js')
	...
	<script>
		...
		const getDetailForUpdate = {
			set loadData(data) {
				const urlDetail = URL_DATA + "/souvenir/" + data
				Functions.prototype.requestDetail(getDetailForUpdate, urlDetail)
			},
			set successData(response) {
				$('#id').val(response.souvenir.id)
				$('#title').val(response.souvenir.title)
				$('#prevThumb').attr('src', PICT + '/thumbnail/' + response.souvenir.thumbnail)
				$('#old_thumb').val(response.souvenir.thumbnail)
				$('#alamat').val(response.souvenir.address)
				$('#latitude').val(response.souvenir.latitude)
				$('#longtitude').val(response.souvenir.longtitude)
				setTimeout(function() {
					tinyMCE.get("desc").setContent(response.souvenir.desc);
				}, 5000);
				// selected tags
				var tagSelect = $('#select_place_tag');
				$.ajax({
					type: 'GET',
					url: URL_DATA + "/souvenir/" + id
				}).then(function (data) {
				...
				var picts = response.souvenir.pictures
				if(picts.length > 0) {
					var listImage = ""
					picts.map(picture => {
						listImage += `
	...
	<script src="{{ asset('js/ownerSouvenir/souvenir/image.js') }}"></script>
	{{-- Select2 --}}
