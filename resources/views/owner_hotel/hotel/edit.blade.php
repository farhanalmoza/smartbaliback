				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header d-flex justify-content-between align-items-center">
							<div class="card-title">Form Edit Hotel</div>
												...
												<div class="col-sm-12">
													...
													<div class="form-group form-inline">
														...
														<div class="col-md-8 p-0">
															<select id="select_hotel_tag" name="tag[]" class="form-control custom-select w-100" multiple>
...
@section('js')
	...
	<script>
		...
		const getDetailForUpdate = {
			...
			set successData(response) {
				$('#id').val(response.hotel.id)
				$('#title').val(response.hotel.title)
				$('#prevThumb').attr('src', PICT + '/thumbnail/' + response.hotel.thumbnail)
				$('#old_thumb').val(response.hotel.thumbnail)
				$('#alamat').val(response.hotel.address)
				$('#latitude').val(response.hotel.latitude)
				$('#longtitude').val(response.hotel.longtitude)
				setTimeout(function() {
					tinyMCE.get("desc").setContent(response.hotel.desc);
				}, 5000);
				// selected tags
				var tagSelect = $('#select_hotel_tag');
				$.ajax({
					type: 'GET',
					url: URL_DATA + "/hotel/" + id
				}).then(function (data) {
	...
	<script src="{{ asset('js/ownerHotel/hotel/image.js') }}"></script>
	{{-- Select2 --}}
