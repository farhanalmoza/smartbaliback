...
class OwnerCarService
{
    ...
    {
        $profile_id = OwnerCar::where('email', $email)->first()->id;
        $profile = OwnerCar::find($profile_id);
    ...
    {
        $result = OwnerCar::where('email', $email);
    ...
    public function updateFoto($file, $email)
    {
        ...
        $profile = OwnerCar::where('email', $email);
        if($profile) {
